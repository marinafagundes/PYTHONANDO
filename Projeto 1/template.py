...
class UI:
    def start(self): # iniciar a aplicação
        while True:
            print('''
            [1] -> Criar conta
            [2] -> Desativar conta
    ...
    def _criar_conta(self):
         ...
         banco = input().title() # NUBANK ou nubank viram Nubank
    ...
    def _filtrar_movimentacoes(self):
        ...
        data_inicio = datetime.strptime(data_inicio, '%d/%m/%Y').date() # converter para data
